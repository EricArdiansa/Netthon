...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="BHP Net Tool", formatter_class=argparse.RawDescriptionHelpFormatter, epilog=textwrap.dedent(''' Example : 
     ...
     echo 'ABC' netcat.py -t 192.168.0.121 -p 135 # echo text to server port 135  
     netcat.py -t 192.168.1.108 -p 5555 # connect to server 
    '''))
    parser.add_argument('-c', '--command', action='store_true', help='command shell')
    parser.add_argument('-e', '--execute', help='execute the spesified command')
    parser.add_argument('-l','--listen',action='store_true', help='listen')
    parser.add_argument('-p','--port', type=int, default=9001, help='spesified port')
    parser.add_argument('-t', '--target', default='192.168.0.121', help='specifed port')
...
class NetCat:
    ...
    def handle(self, client_socket):
        if self.args.execute:
            ...
            file_buffer = b''
            while True:
                data = client_socket.receive(4096)
                if data:
                    file_buffer += data
                else:
                    break
